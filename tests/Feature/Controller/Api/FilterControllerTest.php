...
class FilterControllerTest extends TestCase
{
    public function test_unauthenticated_user_cannot_access_filters_endpoints()
    {
        $this->postJson('/api/filters')
            ->assertUnauthorized();
        $this->putJson('/api/filters/FAKE_FILTER_ID')
            ->assertUnauthorized();
        $this->deleteJson('/api/filters/FAKE_FILTER_ID')
            ->assertUnauthorized();
        $this->putJson('/api/filters/FAKE_FILTER_ID/FAKE_FILTER_VIEW/default')
            ->assertUnauthorized();
        $this->deleteJson('/api/filters/FAKE_FILTER_ID/FAKE_FILTER_VIEW/default')
            ->assertUnauthorized();
    }
    ...
    public function test_user_can_remove_default_filter()
    {
        ...
        $repository->markAsDefault($filter->id, 'dummy-view', $user->id);
