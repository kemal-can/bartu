...
class WebFormRepositoryTest extends TestCase
{
    ...
    {
        parent::setUp();
        $this->repository = app(WebFormRepository::class);
    }
    ...
    public function test_it_can_find_web_form_by_uuid()
    {
        ...
        $this->assertInstanceOf(WebForm::class, $this->repository->findByUuid($form->uuid));
    }

    public function test_it_adds_the_field_to_the_section_when_retrieving_for_display()
    {
        $uuid = WebForm::factory()->addFieldSection('email')->create()->uuid;

        $form = $this->repository->findByUuidForDisplay($uuid);

        $this->assertArrayHasKey('field', $form->sections[0]);
    }

    public function test_web_form_can_be_submitted()
    {
        $this->seedForSubmission();
        ...
        $request = $this->prepareRequestForSubmission($form, [
            ...
            'company_domain' => 'bartucrm.com',
            'companies_file' => UploadedFile::fake()->image('companies_photo.jpg'),
        ]);

        $this->repository->processSubmission($request);
        ...
        $this->assertDatabaseHas('contacts', [
            ...
            'user_id'    => $form->user_id,
            'source_id'  => app(SourceRepository::class)->findByFlag('web-form')->id,
        ]);
    ...
    protected function seedForSubmission()
    {
