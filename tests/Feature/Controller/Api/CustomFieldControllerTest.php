...
class CustomFieldControllerTest extends TestCase
{
    ...
    public function test_optionable_custom_field_requires_options()
    {
        ...
        $this->postJson('/api/custom-fields', [
            'field_type'    => Fields::getOptionableFieldsTypes()[0],
            'field_id'      => 'some_id',
            'label'         => 'Label',
            'resource_name' => 'contacts',
            'options'       => [],
    ...
    public function test_user_can_create_custom_fields()
    {
        ...
        foreach (Fields::getNonOptionableFieldsTypes() as $type) {
            $this->postJson('/api/custom-fields', [
                'field_type'    => $type,
        ...
        foreach (Fields::getOptionableFieldsTypes() as $type) {
            $this->postJson('/api/custom-fields', [
                'field_type'    => $type,
                'field_id'      => $id = 'cf_some_id_' . strtolower($type),
                'label'         => $type,
    ...
    public function test_user_can_update_custom_fields()
    {
        ...
        $field = resolve(CustomFieldRepository::class)->create([
            'field_type'    => 'Checkbox',
