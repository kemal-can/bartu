...
class PipelineRepositoryTest extends TestCase
{
    ...
    {
        parent::setUp();
        $this->repository = app(PipelineRepository::class);
    }
    ...
    public function test_stages_are_created_when_creating_new_pipeline()
    {
        ...
        $pipeline   = $this->repository->create(array_merge($attributes, [
            'stages' => [
    ...
    public function test_it_uses_index_as_display_order_when_display_order_is_not_provided()
    {
        ...
        $pipeline = $this->repository->update([
            'name'   => $pipeline->name,
            'stages' => [
              [
    ...
    public function test_it_can_update_board_default_sort_data()
    {
        $user     = $this->createUser();
        $pipeline = Pipeline::factory()->create();

        $this->repository->updateBoardDefaultSort($pipeline->id, $user->id, ['direction' => 'asc', 'field' => 'created_at']);
    ...
    public function test_it_cannot_delete_primary_pipeline()
    {
        ...
        $this->expectExceptionMessage(__('deal.pipeline.delete_primary_warning'));
    ...
    public function test_it_cannot_delete_pipeline_with_deals()
    {
        ...
        $this->expectExceptionMessage(__('deal.pipeline.delete_usage_warning_deals'));
