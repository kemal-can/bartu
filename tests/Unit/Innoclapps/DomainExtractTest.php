...
'.ve', '.vg', '.vi', '.vn', '.vu', '.wf', '.ws', '.ye', '.yt', '.yu', '.za', '.zm', '.zr', '.zw', '.dev', '.localhost', '.example', '.test', '.invalid', '.digital', '.online', ];
    ...
    public function test_domain_name_extract_works_properly()
    {
        ...
        foreach ($this->tlds as $tld) {
            if (Domain::extractFromUrl('https://bartucrm' . $tld . '/crm') === 'bartucrm' . $tld) {
                $totalMatchesWithSubfolder++;
            }
        }

        foreach ($this->tlds as $tld) {
            if (Domain::extractFromUrl('https://crm.bartucrm' . $tld) === 'bartucrm' . $tld) {
                $totalMatchesWithSubdomain++;
            }
        }

        foreach ($this->tlds as $tld) {
            if (Domain::extractFromUrl('https://app.bartucrm' . $tld . '/crm') === 'bartucrm' . $tld) {
        ...
        // Without https test
        $this->assertEquals(Domain::extractFromUrl('http://crm.bartucrm.com'), 'bartucrm.com');
        $this->assertEquals(Domain::extractFromUrl('crm.bartucrm.com'), 'bartucrm.com');
    }
}
