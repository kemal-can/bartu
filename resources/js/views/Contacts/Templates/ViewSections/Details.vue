...
export default {
  ...
  data: () => ({
    ...
    fieldsHeight: Innoclapps.config.fields.height.contact,
  }),
  props: {
    contact: { required: true, type: Object },
    resourceName: { type: String, default: 'contacts' },
  },
  computed: {
    /**
     * The current record
  ...
  methods: {
    ...
    initRecord() {
      this.getResourceUpdateFields().then(fields => {
        this.prepareComponent(fields, this.contact)
      })
    },
  },
  beforeMount() {
